...
async function loadData() {
    try {
        const response = await fetch('estudo_aprofundado.json');
...
function openSearchResultByIndex(index) {
    if (window.currentSearchResults && window.currentSearchResults[index]) {
        showContent(window.currentSearchResults[index].title);
    }
}
...
function showContent(title) {
    ...
    // Metadados - Removido conforme solicitado (botões não funcionais)
